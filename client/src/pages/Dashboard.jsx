        ...
        <div className='max-w-7xl mx-auto px-4 py-8'>
            ...
            <div className='flex gap-4'>
                ...
                <button onClick={()=> setShowUploadResume(true)} className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2
                 text-slate-600 border border-dashed border-slate-300 group hover:border-purple-500 hover:shadow-lg transition-all 
                    duration-300 cursor-pointer'>
            ...
            <div className='grid grid-cols-2 sm:flex flex-wrap gap-4'>
                {allResumes.map((resume, index) => {
                    ...
                    return (
                        <button onClick={() => navigate(`/app/builder/${resume._id}`)} key={index} className='relative w-full sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg
            ...
            {showCreateResume && (
                ...
                <div onClick={(e) => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg p-6 w-full max-w-md'>
                    <h2 className='text-xl font-bold mb-4'>Create New Resume</h2>
                    <input onChange={(e) => setTitle(e.target.value)} value={title} type="text" placeholder="Enter Resume Title" className='px-4 py-2 mb-4 focus:border-green-300 ring-green-600 w-full' required />
            ...
            {showUploadResume && (
                ...
                <div onClick={(e) => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg p-6 w-full max-w-sm'>
                    <h2 className='text-xl font-bold mb-4'>Upload Resume</h2>
                    <input onChange={(e) => setTitle(e.target.value)} value={title} type="text" placeholder="Enter Resume Title" className='px-4 py-2 mb-4 focus:border-green-300 ring-green-600 w-full' required />

                    <div>
                        <label htmlFor="resume-input" className='block text-sm font-bold text-slate-700'>Select resume file
                            <div className='flex flex-col items-center justify-center gap-2 border group text-slate-400 border-slate-400 broder-dashed rounded-md p-4 my-4 hover:border-green-500 hover:text-green-700 cursor-pointer transition-colors'>
                                {resume ? (
                                    <p className='text-green-600'>Selected: {resume.name}</p>
                                ) : (
                                    <>
                        ...
                        <input onChange={(e) => setResume(e.target.files[0])} type ="file" id='resume-input' accept='.pdf,.doc,.docx' hidden />
                    </div>
            ...
            {editResumeId && (
                ...
                <div onClick={(e) => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg p-6 w-full max-w-md'>
                    ...
                    <button className='w-full bg-green-600 text-white py-2 rounded hover:bg-green-700 transition-colors'>Update Resume</button>
